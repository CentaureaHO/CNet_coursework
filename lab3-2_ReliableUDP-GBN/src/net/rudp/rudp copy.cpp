...
#define PROCESS_GAP 20000      // 给接收端20ms处理时间
...
void RUDP::_resendLoop()
{
    while (true)
    {
        ...
        timeout          = us(static_cast<long long>(_rtt.count() * BASE_TIMEOUT_FACTOR));
        WriteGuard guard = _buffer_lock.write();
        if (_send_buffer.empty()) continue;

        CLOG_WARN(statuStr(_statu), ": Timeout, resend all packets in the buffer.");
        for (auto& [seq_num, packet_timer] : _send_buffer)
        {
            ...
            CLOG_WARN(statuStr(_statu), ": Resend packet ", packet_timer.packet.header.seq_num);
...
void RUDP::_receiveHandler()
{
    ...
    while (true)
    {
...

        {
            ...
            auto item = ack_cnt_map.find(recv_buffer.header.ack_num);
            if (item == ack_cnt_map.end())
                ack_cnt_map[recv_buffer.header.ack_num] = 1;
            else
                ++item->second;
            ...
            if (ack_cnt_map[recv_buffer.header.ack_num] >= 3)
            {
...
void RUDP::_server_receiveHandler(Callback cb)
{
    ...
    while (true)
    {
        ...
        if (ack_cnt >= 3 || cur_time - last_send_time >= us(static_cast<long long>(_rtt.count())))
        {
            if (ack_cnt >= 3) SLOG(statuStr(_statu), ": Received 5 packets, send ACK packet.");
